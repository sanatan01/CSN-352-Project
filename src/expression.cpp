...
Expression::Expression(PrimitiveTypes type, int num_operands): prim_type(static_cast<int>(type)), num_operands(num_operands), is_assignable(true), is_constant(false), exp_type(create_primitive_type(type)) {};
Expression::Expression(): prim_type(static_cast<int>(ERROR_T)), num_operands(0), is_assignable(true), is_constant(false), exp_type(create_invalid_type("Invalid expression", line_num, column)) {};

Expression::Expression(class GlobalType* type): prim_type(static_cast<int>(ERROR_T)), num_operands(0), exp_type(type), is_assignable(true), is_constant(false) {
    if (type == nullptr) {
...
PrimitiveTypes greater_type(PrimitiveTypes op1, PrimitiveTypes op2) {
    if (op1 == ERROR_T || op2 == ERROR_T) {
        return ERROR_T;
    }
    if (op1 == BOOL_T) {
        return op2;
...
std::string convert_to_int_value(std::string token) {
    // Handle character literals
    if (token[0] == '\'' && token[token.size() - 1] == '\'') {
        if (token.size() == 3) {
            // Simple character: 'a'
            return std::to_string(static_cast<int>(token[1]));
        }
            ...
            char escaped_char = escape_chars.count(token[2]) ?
                escape_chars.at(token[2]) : token[2];
            return std::to_string(static_cast<int>(escaped_char));
        }
    }

    if (token.size() > 2 && token[0] == '0' && (token[1] == 'x' || token[1] == 'X')) {
        return std::to_string(std::stoll(token, nullptr, 16)); // Hex
    }
    else if (token.size() > 1 && token[0] == '0') {
        return std::to_string(std::stoll(token, nullptr, 8));  // Octal
    }
    else {
        return std::to_string(std::stoll(token, nullptr, 10)); // Decimal
    }
}
...
std::string operation_float_value(std::string op, std::string op1, std::string op2) {
    if (op == "+") {
    ...
    if (oe->op == "*" || oe->op == "/") {
        ...
        if (oe->op1.is_constant && oe->op2.is_constant) {
            ...
            if (isInt(tmp))
                ...
                oe->name = operation_float_value(oe->op, oe->op1.name, oe->op2.name);
            return oe;
        }
...
Expression* create_expression_simple(ExpressionType exp_type, std::string token) {
    ...
    case CONSTANT_ET:
    {
        ...
        e->prim_type = type;
        if (isInt(type)) {
            e->name = convert_to_int_value(token);
        }
        else {
            e->name = token;
        }
        ...
        e->is_constant = true;
        debug_msg("New constant found " + token);
        return e;
        break;
    }
    case STRING_ET:
